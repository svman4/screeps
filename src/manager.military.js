...
var militaryController = {
    ...
    run: function (roomName) {
        ...
        const towers = roomCache.in(roomName).groupedStructures[STRUCTURE_TOWER] || [];
    ...
    calculateDefensePower: function (room, towers) {
        ...
        const militaryCreeps = roomCache
            .in(room.name)
            .myCreeps
            .filter(
                creep => creep.memory.role &&
                    creep.memory.role.includes('military'));
    ...
    emergencyResponse: function (room) {
        ...
        const spawns = roomCache.in(room.name).groupedStructures[STRUCTURE_SPAWN] || [];
    ...
    createScout: function (room) {
        ...
        availableSpawns[0].spawnCreep(body, name, {
            memory: {
                role: 'military_scout',
                homeRoom: room.name,
                mission: 'patrol'
    ...
    manageMilitaryCreeps: function (room) {
        const hostiles = roomCache.in(room.name).hostileCreeps;
        const militaryCreeps = roomCache.in(room.name).myCreeps.filter(creep => creep.memory.role && creep.memory.role.includes('military'));
    ...
    patrol: function (creep) {
        ...
        const patrolPoints = roomCache.in(room.name).patrolPoints;
