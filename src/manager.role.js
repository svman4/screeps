...
function smartMove(creep, target, range = 1, reuse = 20) {
    if (creep.pos.inRangeTo(target, range)) return OK;
    return creep.moveTo(target, { reusePath: reuse });
}
...
const roleManager = {
    ...
    repairStructures: function (creep) {
        ...
        const repairStructures = structures[0];
        if (!repairStructures) { return false; }
        if (creep.pos.inRangeTo(repairStructures, 3)) {
            creep.repair(repairStructures);
        } else {
            creep.moveTo(repairStructures, { visualizePathStyle: { stroke: '#00ff00' }, reusePath: 8 });
        ...
        if (controller) {
            ...
            if (!controller.my) {
                ...
                if (claimResult === ERR_NOT_IN_RANGE) {
                    smartMove(creep, controller, 1, 20);
                ...
                if (creep.memory.building) {
                    ...
                    if (targetSite) {
                        if (creep.pos.inRangeTo(targetSite, 3)) {
                            ...
                            smartMove(creep, targetSite, 3, 30);
                    ...
                    if (controller.my || controller.reservation) {
                        if (creep.pos.inRangeTo(controller, 3)) {
                            ...
                            smartMove(creep, controller3, 30);
    ...
    fillContainerOrStorage: function (creep) {
        ...
        if (targets && targets.length > 0) {
            if (creep.pos.inRangeTo(targets[0], 1)) {
                ...
                smartMove(creep, targets[0], 1, 20);
        ...
        if (constructionSites.length > 0) {
            ...
            if (closestSite) {
                if (creep.pos.inRangeTo(closestSite, 3)) {
                    ...
                    smartMove(creep, closestSite, 3, 20);
        ...
        if (constructionRoad.length > 0) {
            ...
            if (closestRoad) {
                if (creep.pos.inRangeTo(closestRoad, 3)) {
                    ...
                    smartMove(creep, closestRoad, 3, 20);
        ...
        if (sources.length > 0) {
            ...
            if (closestSource) {
                if (creep.pos.inRangeTo(closestSource, 1)) {
                    ...
                    smartMove(creep, closestSource, 1, 30);
    ...
    upgradeController: function (creep) {
        if (creep.pos.inRangeTo(creep.room.controller, 2)) {
            ...
            smartMove(creep, creep.room.controller, 2, 30);
...
function runRecycleCreep(creep) {
    ...
    if (recycleContainer && !creep.pos.inRangeTo(recycleContainer, 0)) {
        smartMove(creep, recycleContainer, 0, 30);
